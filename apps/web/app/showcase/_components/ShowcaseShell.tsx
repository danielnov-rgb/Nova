...
import { initPostHog, identifyUser, trackEvent } from "../_lib/posthog";

const SHOWCASE_PASSWORD = process.env.NEXT_PUBLIC_SHOWCASE_PASSWORD || "nova2025";
...
export function ShowcaseShell({ children }: ShowcaseShellProps) {
  ...
  useEffect(() => {
    initPostHog();
    const stored = sessionStorage.getItem("showcase_auth");
    if (stored) {
      try {
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    sessionStorage.setItem("showcase_auth", JSON.stringify({ name: name.trim(), timestamp: Date.now() }));
    identifyUser(name.trim());
    trackEvent("showcase_viewed", { name: name.trim() });
    setAuthenticated(true);
  }
